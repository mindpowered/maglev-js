...
class maglev_MagLev {
	...
	call(method,args) {
		if(Object.prototype.hasOwnProperty.call(this._methods.h,method)) {
			...
			while(_g < args.length) {
				let arg = args[_g];
				++_g;
				args2.push(maglev_MagLev.clean(arg));
			}
			return this._methods.h[method](args2);
		} else {
			throw haxe_Exception.thrown("Method '" + method + "' not registered");
		}
	}
	...
	static clean(arg) {
		if(typeof(arg) != "string" && Reflect.isObject(arg) && !js_Boot.__implements(arg,haxe_IMap)) {
			let keys = Reflect.fields(arg);
			let map = new haxe_ds_StringMap();
			let _g = 0;
			while(_g < keys.length) {
				let key = keys[_g];
				++_g;
				let val = Reflect.field(arg,key);
				let v = maglev_MagLev.clean(val);
				map.h[key] = v;
			}
			return map;
		} else {
